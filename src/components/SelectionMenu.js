...
export function SelectionMenu({
  ...
  const faction_lists = {
    "Good Army": new Set(mesbg_data
      ...
      .map((data) => data.faction)), "Evil Army": new Set(mesbg_data
      .filter((data) => data.faction_type === "Evil Army")
      .map((data) => data.faction)), "Good LL": new Set(mesbg_data
      .filter((data) => data.faction_type === "Good LL")
      .map((data) => data.faction)), "Evil LL": new Set(mesbg_data
  ...
  >
    {displaySelection ? (<>
      ...
      <Tabs activeKey={tabSelection} fill onSelect={setTabSelection}>
        {Object.keys(faction_lists).map((f_type) => (<Tab
          eventKey={f_type}
          title={f_type}
          disabled={!heroSelection || (factionType !== "" && factionType !== f_type)}
        >
          <Stack gap={2}>
            <DropdownButton
            ...
            {heroSelection ? mesbg_data
              .filter((data) => data.faction === factionSelection[f_type] && !["Independent Hero*", "Warrior"].includes(data.unit_type) && !(data.unique && uniqueModels.includes(data.model_id)))
              .map((row) => (<SelectionUnit
                key={uuid()}
              ...
              />)) : mesbg_data
              .filter((data) => roster.warbands[warbandNumFocus].hero && hero_constraint_data[roster.warbands[warbandNumFocus].hero.model_id][0]["valid_warband_units"].includes(data.model_id) && !(data.unique && uniqueModels.includes(data.model_id)))
              .map((row) => (<SelectionUnit
                key={uuid()}
                newWarriorFocus={newWarriorFocus}
                setDisplaySelection={setDisplaySelection}
