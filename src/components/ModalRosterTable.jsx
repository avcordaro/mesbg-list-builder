          ...
          <Modal.Title className="w-100">
            <Stack direction="horizontal" gap={3}>
              ...
              <Button className="me-3" onClick={() => downloadProfileCards()}>
                {downloadSpinner ? (<Spinner size="sm" animation="border"/>) : (<FaDownload/>)}{" "}
                Profile Cards
              </Button>
            </Stack>
          </Modal.Title>
        ...
        <Modal.Body id="rosterList">
          {!textView ? (<>
            ...
            <Table style={{verticalAlign: "middle"}} size="sm" bordered>
              ...
              <tbody>
              {roster.warbands.map((warband) => (<>
                {warband.hero != null && (<tr className={warband.num % 2 === 0 ? "secondary" : ""}>
                  <td
                    style={{
                  ...
                  >
                    <b>{warband.num}</b>{" "}
                    {roster["leader_warband_num"] === warband.num ? (<span>
                              - <GiQueenCrown/> Leader
                            </span>) : ("")}
                  </td>
                  <td
                  ...
                  >
                    {warband.hero.options
                      .map((option) => option.opt_quantity > 0 ? option.max > 1 ? option.opt_quantity + " " + option.option : option.option : "")
                      .filter((opt) => opt !== "")
                      .join(", ")}
                  </td>
                  <td
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            ...
            <h6>
              The following is a screenshot image of your roster list which you can download, or copy to your clipboard as you wish.
            </h6>
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <div className="screenshot m-3">
            {screenshot != null &&
              <img className="border shadow border-secondary" src={screenshot} alt=""/>
            }
          </div>
        </Modal.Body>
      </Modal>
