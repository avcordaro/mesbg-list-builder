...
export const handleSpecialArmyOption = (newRoster, warbandNum) => {
  ...
  let specialArmyOption =
    hero_constraint_data[newRoster.warbands[warbandNum - 1].hero.model_id][0][
      "special_army_option"
    ];
  newRoster.warbands = newRoster.warbands.map((warband) => {
    let newWarband = { ...warband };
    newWarband.units = newWarband.units.map((_unit) => {
      ...
      if (newUnit.name != null) {
        newUnit.options = newUnit.options.map((_option) => {
          let newOption = { ..._option };
          if (
            newOption.type === "special_army_upgrade" &&
            newOption.opt_quantity === 1 &&
            newOption.option === specialArmyOption
          ) {
            newRoster["points"] = newRoster["points"] - newUnit["pointsTotal"];
            newWarband["points"] =
              newWarband["points"] - newUnit["pointsTotal"];
            newUnit["pointsPerUnit"] =
              newUnit["pointsPerUnit"] - newOption.points;
            newUnit["pointsTotal"] =
              newUnit["pointsPerUnit"] * newUnit["quantity"];
            newWarband["points"] =
              newWarband["points"] + newUnit["pointsTotal"];
            newRoster["points"] = newRoster["points"] + newUnit["pointsTotal"];
      ...
      newWarband.units = newWarband.units.map((_unit) => {
        ...
        if (newUnit.name != null) {
          newUnit.options = newUnit.options.map((_option) => {
            let newOption = { ..._option };
            if (
              newOption.type === "special_warband_upgrade" &&
              newOption.opt_quantity === 1
            ) {
              newRoster["points"] =
...
export const handle50PctBowLimit = (faction_data, alliance_level) => {
  ...
  new_faction_data["The Serpent Horde"].bow_limit =
    alliance_level === "Historical" ? 0.5 : 0.33;
  new_faction_data["Azog's Hunters"].bow_limit =
    alliance_level === "Historical" ? 0.5 : 0.33;
  return new_faction_data;
};

export const checkSiegeEngineCounts = (
  siegeEngines,
  heroicTiers,
  _warnings,
) => {
  Object.keys(heroicTiers).map((faction) => {
    if (siegeEngines[faction] > 0) {
      let heroForts = heroicTiers[faction].reduce(
        (n, v) =>
  ...
  if (Object.keys(heroicTiers).length > 1) {
    Object.keys(heroicTiers).map((faction) => {
      ...
      if (_allianceLevel === "Historical") {
        ...
        let heroValours = heroicTiers[faction].reduce(
          (n, v) =>
