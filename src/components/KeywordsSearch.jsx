...
export function KeywordsSearch({ showKeywordSearch, setShowKeywordSearch }) {
  ...
  const handleSearch = (e) => {
    ...
    let newKeywordList = keywords.filter((kw) =>
  ...
  return (
    <Offcanvas
      show={showKeywordSearch}
      onHide={() => setShowKeywordSearch(false)}
    >
      <Offcanvas.Header className="border border-secondary" closeButton>
        <Offcanvas.Title>
          <SiCodemagic className="me-2" style={{ fontSize: "26px" }} /> Search
          Keywords
        </Offcanvas.Title>
      </Offcanvas.Header>
      <Offcanvas.Body>
        <Accordion alwaysOpen>
          <div className="mt-3">
            Use the search bar below to find definitions for special rules,
            heroic actions and magical powers that you see on profile cards.
          </div>
